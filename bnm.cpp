...
double bnm(int nodes, u_real * SC, gsl_matrix * SC_gsl,
    ...
    /*
     Runs a simulation of reduced Wong Wang model
    */
    ...
    u_real *_w_EE = (u_real *)malloc(nodes * sizeof(u_real));
    u_real *_w_EI = (u_real *)malloc(nodes * sizeof(u_real));
    u_real *_w_IE = (u_real *)malloc(nodes * sizeof(u_real));
    ...
    if (w_IE[0]==0) {
        ...
        // between the OMP threads as some variables used in FIC are global variables
        if (_unstable) {
            printf("FIC solution is unstable. Setting wIE to 1 in all nodes\n");
            for (int j=0; j<nodes; j++) {
                _w_IE[j] = 1.0;
            }
        } else {
    ...
    while (ts_bold <= time_steps) {
        ...
        // Balloon-Windkessel step
        for (j=0; j<nodes; j++) {
            bw_x[j]  = bw_x[j]  +  model_dt * (S_i_E[j] - kappa * bw_x[j] - y * (bw_f[j] - 1.0));
            tmp_f       = bw_f[j]  +  model_dt * bw_x[j];
            bw_nu[j] = bw_nu[j] +  model_dt * itau * (bw_f[j] - POW(bw_nu[j], ialpha));
            bw_q[j]  = bw_q[j]  +  model_dt * itau * (bw_f[j] * (1.0 - POW(oneminrho,(1.0/bw_f[j]))) / rho  - POW(bw_nu[j],ialpha) * bw_q[j] / bw_nu[j]);
            bw_f[j]  = tmp_f;   
        }
        if (ts_bold % BOLD_TR == 0) {
            for (j = 0; j<nodes; j++) {
                bold_idx = BOLD_len_i*nodes+j;
                BOLD_ex[bold_idx] = V_0 * (k1 * (1 - bw_q[j]) + k2 * (1 - bw_q[j]/bw_nu[j]) + k3 * (1 - bw_nu[j]));
                if (extended_output) {
...
{
    /*
        Runs a group of reduced Wong Wang simulations (in parallel if possible)
    */
    #pragma omp parallel
	#pragma omp for
